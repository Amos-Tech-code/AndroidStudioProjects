...
fun PopularMovieScreen(
    ...
    if (movieListState.popularMoviesList.isEmpty()) {
        Box(
        ...
        LazyVerticalGrid(
            ...
            items(movieListState.popularMoviesList.size) { index ->
                MovieItem(
                    movie = movieListState.popularMoviesList[index],
                    navHostController =navController
                )

                Spacer(modifier = Modifier.height(16.dp))

                if (index >= movieListState.popularMoviesList.size - 1 && !movieListState.isLoading) {
