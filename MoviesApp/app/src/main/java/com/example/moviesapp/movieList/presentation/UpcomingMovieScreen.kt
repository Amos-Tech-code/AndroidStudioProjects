...
fun UpcomingMovieScreen(
    ...
    if (movieListState.upcomingMoviesList.isEmpty()) {
        Box(
        ...
        LazyVerticalGrid(
            ...
            items(movieListState.upcomingMoviesList.size) { index ->
                MovieItem(
                    movie = movieListState.upcomingMoviesList[index],
                    navHostController =navController
                )

                Spacer(modifier = Modifier.height(16.dp))

                if (index >= movieListState.upcomingMoviesList.size - 1 && !movieListState.isLoading) {
